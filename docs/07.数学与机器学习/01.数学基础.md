...
::: tip 这篇笔记的定位（标注于 2026-09，正文未改动）
**这是一篇「训练导向」的数学笔记**，主线是：梯度 → 矩阵求导 → 梯度矩阵 → 极大似然估计。
目的是理解**模型是怎么被训练出来的**。内容扎实，**不要重写**。

**如果你的目标是 Agent 工程（RAG / 评测 / 推理部署），只需要其中两节：**

| 需要的 | 位置 | 对应 |
|---|---|---|
| 标量与向量 → 向量运算 / 向量范数 | 「线性代数」章前 120 行 | M-1 闸门 1 |
| 偏导数 / 梯度 | 「高数」章「偏导数和梯度」一节 | M-1 闸门 2 |

**以下几节与 Agent 工程基本无关，可直接跳过**：方向导数、矩阵的逆、张量、矩阵求导、梯度矩阵、克罗内克积。

**这篇缺失、但在 Agent 工程里天天要用的概念**，已另建可填空笔记，见
`08.AI应用与Agent/06.Agent工程/01.数学直觉/`：
余弦相似度与 embedding 几何 · 交叉熵与困惑度 · 置信区间与样本均值分布 · 温度与 top-p · 浮点精度与量化。

一句话：**训练要的是「梯度怎么传」，推理与检索要的是「向量怎么比、概率怎么采」。这篇只覆盖了前者。**
:::
